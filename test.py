...
import numpy as np
from dataloader import testset_list_classifier ,testset_loader
from train import reparameterize, get_z_random
...
def test():
    ...
    list_dir_A = './etc/Test_et05_real_isolated_1ch_track_list.csv'
    list_dir_B = './etc/Test_et05_simu_isolated_1ch_track_list.csv'

    output_dir = './output/{}/{}_AB_dt'.format(hparams.task_name, hparams.iteration_num)
    output_dir_real = os.path.join( output_dir, 'et_real')
    output_dir_simu = os.path.join( output_dir, 'et_simu')
    ...
    for i in range(len(test_list_A)):
        ...
        np.save( os.path.join( output_dir_real, feat['audio_name']), feature_z)
    ...
    for i in range(len(test_list_B)):
        ...
        np.save( os.path.join( output_dir_simu, feat['audio_name']), feature_z)
